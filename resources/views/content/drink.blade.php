...
@section('content')
    <body>
                ...
                <div class="row align-items-center">
                    <div class="col-sm-12">
                        <div class="breadcrumbs-area ">
                            <h4 class="page-title pull-left mt-3">Drinks</h4>
                        </div>
                        ...
                        <tbody>
                            @foreach ($drink as $drinks)
                                <tr>
                                    <td>{{$no++}}</td>
                                    <td>{{$drinks->name}}</td>
                                    <td>Rp.{{number_format($drinks->price)}}</td>
                                    <td>{{$drinks->status}}</td>
                                    <td style="text-align: center">
                                        <a data-toggle="modal" data-target="#editModal-{{$drinks->id}}" class="btn btn-warning"><i class="fas fa-edit" style="color: #fff;"></i></a>
                                        <a class="btn btn-danger ml-3" href="/delete-drinks/{{ $drinks->id }}"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
        ...
        @foreach ($drink as $edt)
            ...
            <div class="modal fade" id="editModal-{{$edt->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <h5 class="modal-title" id="exampleModalLabel">Edit Drinks</h5>
        ...
        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Add Drinks</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
                </div>
                <div class="modal-body" style="text-align: center">
                    <form action="/create-drink" method="POST" enctype="multipart/form-data" autocomplete="off">
                        {{ csrf_field() }}
